...
const NavbarChildChild = ({sub, getCookie, setToggle, handlerTop}) => {
    ...
    return(
        <li onClick={closeTogChild} className="menu-item menu-item-has-children" >
            ...
            {sub.children && sub.children.map((subChild, subNumber) =>
                    <NavChildChildChild subChild={subChild} key={subNumber} getCookie={getCookie} setToggle={setToggle} handlerTop={handlerTop}/>
            )}
            </ul>
        </li>
    )
