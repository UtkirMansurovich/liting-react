...
const NavbarChildChild = ({sub, getCookie, setToggle, handlerTop, selectFontBig, selectFontSmall}) => {
  ...
  const closeTogChild = () => {
    ...
    return(
        <li onClick={closeTogChild} className="menu-item menu-item-has-children" >
            ...
            <ul style={openTogChild ? {display:'block'} : {display:'none'}} className="custom sub-menu">
            {sub.children && sub.children.map((subChild, subNumber) =>
                    <NavChildChildChild subChild={subChild} key={subNumber} getCookie={getCookie} setToggle={setToggle} handlerTop={handlerTop} selectFontBig={selectFontBig} selectFontSmall={selectFontSmall}/>
            )}
            </ul>
        </li>
    )
